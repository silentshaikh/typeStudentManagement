...
class Student {
  ...
  addStudent(name: string, id: string, course: string, feesPaid: number): void {
    this.students.push({
      name: name,
      id: id,
      course: course,
...
while (isCondition) {
  ...
  let notAddDuplicateId: OurStudents | undefined = ourStudents.students.find(
    (e) => {
        ...
        if (fingId.payFees < fingId.coursBalance) {
          ...
          console.log(
            chalk.bgGreen(
              `\n Dear ${chalk.bold(
                fingId.name
              )}, Thank You so much for pay full fee of your ${
                fingId.course
              } course.\n`
            )
          );
